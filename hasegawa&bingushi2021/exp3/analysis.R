# List up files
files = list.files('./cross_rival_stereopsis/data',full.names=T)
f = length(files)

si = toupper(gsub(".*(.)2019.*","\\1", files))
n = length(table(si))
...
plot(x=temp$disparity, y=temp$cdt, ylim=c(0,30), type="p", xlab="horizontal disparity(min of arc)", ylab="mean of cdt(sec)")
par(new=T)
plot(mean, type="l", col="blue", ylim=c(0,30), xlab="", ylab="")
par(new=F)
...
for (i in 1:n){
  ...
  plot(camp, ylim=c(0,30), type="p", xlab="horizontal disparity(min of arc)", ylab="cdt(sec)", main=si[i])
  par(new=T)
  m = aggregate(x=camp$cdt, by=camp["disparity"], FUN=mean)
  pm = cbind(pm, m[2])
  plot(m, type="l", col="blue", ylim=c(0,30), xlab="", ylab="")
  par(new=F)
}
